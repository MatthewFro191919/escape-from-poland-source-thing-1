...
class PlayState extends MusicBeatState
{
	...
	//public static var mainaNotesPS:Int = 6; // this didn't work out good
	...
	var kadeEngineWatermark4:FlxText;
	...
	var bambiTween:ColorTween; // idk might cause errors lol
	...
	// I think I need this
	var whyDoINeedThis:Array<FlxSprite> = [];
	...
	var flyingBgsFarm:FlxTypedGroup<FlyingBGChar> = new FlxTypedGroup<FlyingBGChar>();
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			case 'exploitation':
				...
				sh_r = 600;
			case 'importumania' | 'too-slow-encore': // I'm sure this will make it so it won't break stuff
				FlxG.save.flush();
				modchart = ExploitationModchartType.None;
			case 'recursed':
		...
		//trace(SONG.mania);
		//trace(Main.keyAmmo);
		if (FlxG.save.data.mainaNotesLoly > 0) {
		//	trace('buh....');
		//	trace(Main.keyAmmo[FlxG.save.data.mainaNotesLoly]);
		//	trace(FlxG.save.data.mainaNotesLoly);
			mania = FlxG.save.data.mainaNotesLoly; // this took a lot of time to work
		}
		//trace(Main.keyAmmo[mania]);
		...
		pMode = FlxG.save.data.pMode;
	//	mainaNotesPS = FlxG.save.data.mainaNotesLoly;
		foePlay = FlxG.save.data.opponent;
		...
		var noGFSongs = ['memory', 'five-nights', 'bot-trot', 'escape-from-california', 'overdrive', 'too-slow-encore', 'Old-Too-Slow', 'sporting'];
		...
		if (noGFSongs.contains(SONG.song.toLowerCase()) || !['none', 'bf', 'bf-pixel', 'bf-3d'].contains(formoverride))
		{
			gfVersion = 'gf-none';
		}
	...
	//	} else {
	//	dad = new Character(100, 450, SONG.player1);
	//	}
		switch (SONG.song.toLowerCase())
		{
			case 'insanity':
		...
		if (darkLevels.contains(curStage) && SONG.song.toLowerCase() != "polygonized" && SONG.song.toLowerCase() != "too-slow-encore" || SONG.song.toLowerCase() == 'rano')
		{
		...
		switch (stageCheck)
		{
			...
			case 'banana-hell':
				...
				dad.y -= -70;
			case 'omission':
				dad.setPosition(-10, 250);
				boyfriend.setPosition(620, 260);
				gf.setPosition(350, -50);
			case 'house-older':
				dad.y -= 100;
			case 'garrettLand':
		...
		healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, inFiveNights ? LEFT_TO_RIGHT : RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			...
			healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, foePlay ? LEFT_TO_RIGHT : RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			'displayedHealth', 0, maxHealth);
		}
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'kabunga':
				...
				case 'omission':
				credits = 'Hello I\'m Dan :D :) Welcome ' + boyfriend.curCharacter + '!';
			default:
				credits = '';
		} 
		var creditsText:Bool = credits != '';
		var randomThingy:Int = FlxG.random.int(0, 24);
		...
		if (!isGreetingsCutscene)
		{
			...
			kadeEngineWatermark2.alpha = 0.7;
			...
			kadeEngineWatermark3.alpha = 0.7;
			add(kadeEngineWatermark3);

			kadeEngineWatermark4 = new FlxText(4, textYPos + -60, 0, "Plus Reformed vAlpha", 16);
	
			kadeEngineWatermark4.setFormat(font, 16 * fontScaler, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			kadeEngineWatermark4.scrollFactor.set();
			kadeEngineWatermark4.borderSize = 1.25 * fontScaler;
			kadeEngineWatermark4.antialiasing = FlxG.save.data.globalAntialiasing;
			kadeEngineWatermark4.visible = !FlxG.save.data.hideHud;
			kadeEngineWatermark4.alpha = 0.7;
			add(kadeEngineWatermark4);
		}
		if (creditsText)
		{
		...
		switch (curSong.toLowerCase())
		{
			...
			case 'eletric-cockadoodledoo':
				...
				preload('expunged/ExpungedFinal');
				preload('dave/characters/Dave_insanity_lol');
				case 'sporting':
				preload('expunged/unfair_bambi');
		...
		botplayTxt = new FlxText(400, songPosBar.y + 55, FlxG.width -980, "CPU", 32);
		/*botplayTxt = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (FlxG.save.data.downscroll ? 100 : -100), 0,
		"BOTPLAY", 20);*/
		botplayTxt.setFormat((SONG.song.toLowerCase() == "overdrive") ? Paths.font("ariblk.ttf") : font, 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		botplayTxt.scrollFactor.set();
		...
		if (kadeEngineWatermark != null)
		{
			...
			kadeEngineWatermark4.cameras = [camHUD];
		}
	...
	public function createBackgroundSprites(bgName:String, revertedBG:Bool):FlxTypedGroup<BGSprite>
	{
		...
		if (SONG.song.toLowerCase() == "bananacore" || SONG.song.toLowerCase() == "electric-cockaldoodledoo" || SONG.song.toLowerCase() == "eletric-cockadoodledoo")
			...
			if (SONG.song.toLowerCase() == "importumania" || SONG.song.toLowerCase() == "too-slow-encore") // Bambi
			{
				
				var bagType = FlxG.random.int(0, 1000) == 0 ? 'popeye' : 'cornbag';

				if (SONG.song.toLowerCase() == "too-slow-encore") { // eeeeeeeeeeeeeeeee nice
					var skyType:String = stageName == 'bambiFarmNight' ? 'sky_night' : stageName == 'bambiFarmSunset' ? 'sky_sunset' : 'sky';
					PlayState.storyWeek = 16; // work
					var bg:BGSprite = new BGSprite('bg', -600, -200, Paths.image('backgrounds/shared/' + skyType), null, 0.6, 0.6);
					//sprites.add(bg);
					whyDoINeedThis.push(bg); // AHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHh
					add(bg); 

					for (bg in ['gm_flatgrass', 'orangey hills', 'funfarmhouse', 'grass lands', 'cornFence', 'cornFence2', 'sign', bagType])
						{
							var bgs = new FlyingBGChar(bg, Paths.image('backgrounds/farm/$bg'));
							sprites.add(bgs);
							flyingBgsFarm.add(bgs);
						}
						for (bam in ['bambi'])
							{
								var bam = new FlyingBGChar(bam, Paths.image('backgrounds/festival/scaredCrowd/$bam'));
								sprites.add(bam);
								flyingBgsFarm.add(bam);
							}
						flyingBgsFarm.visible = false;
						add(flyingBgsFarm);
				}
			...
			if (SONG.song.toLowerCase() == "sporting") // Cover
			{
				expungedBG = new BGSprite('void', -600, -200, '', null, 1, 1, false, true);
				        expungedBG.loadGraphic(Paths.image('backgrounds/void/scarybg'));
				        expungedBG.setPosition(0, 200);
				        expungedBG.setGraphicSize(Std.int(expungedBG.width * 3));
				        expungedBG.scrollFactor.set();
				        expungedBG.antialiasing = false;
						expungedBG.visible = false;
				        sprites.add(expungedBG);
				        add(expungedBG);
				        voidShader(expungedBG);
			} 
	...
	private function updateCompactNumbers():Void
		{
			...
			compactMisses = formatCompactNumberInt(misses);
	...
	function startCountdown():Void
	{
		...
		if (SONG.song.toLowerCase() == 'cheating' || SONG.song.toLowerCase() == 'unfairness' || SONG.song.toLowerCase() == 'exploitation')
			{
				if(botPlay || pMode && !FlxG.save.data.devMode)
				{
					FlxG.switchState(new SusState()); // funky jumpscare lol
				}
			}
...
//		trace("Generated FilePath: " + (Sound.fromFile(Paths.inst(PlayState.SONG.song))));
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'too-slow-encore')
			{
				var speed = 300;
				flyingBgsFarm.forEach(function(bFD:FlyingBGChar)
				{
					var moveDir = bFD.direction == 'left' ? -1 : bFD.direction == 'right' ? 1 : 0;
					bFD.x += speed * elapsed * moveDir * bFD.randomSpeed;
					bFD.y += (Math.sin(elapsedtime) * 5);
		
					bFD.angle += bFD.angleChangeAmount * elapsed;
	
					switch (bFD.direction)
					{
						case 'left':
							if (bFD.x < bFD.leftPosCheck)
							{
								bFD.switchDirection();
							}
						case 'right':
							if (bFD.x > bFD.rightPosCheck)
							{
								bFD.switchDirection();
							}
					}
      ...
      if (stageCheck == 'exbungo-land') {
		...
		if (dad.curCharacter == 'recurser' || dad.curCharacter == 'dan' || dad.curCharacter == 'dan-v2' )
		{
			...
			if((SONG.song.toLowerCase() == 'omission'))
				{
		...
		if(funnyFloatyBoys.contains(gf.curCharacter.toLowerCase()) && canFloat || (SONG.song.toLowerCase() == 'omission'))
		{
		...
		if (modchartoption) {
			...
			if (SONG.song.toLowerCase() == 'importumania' && !inCutscene && mcStarted) // fuck you
				...
				if (SONG.song.toLowerCase() == 'too-slow-encore' && !inCutscene && mcStarted) // fuck you
					{
							switch (modchart)
							{
								case ExploitationModchartType.None:
			...
			switch (SONG.song.toLowerCase())
			{
				case 'overdrive':
					...
					default:
						...
						scoreTxt.text =
						if (FlxG.save.data.compactNumbers) {
						(FlxG.save.data.npsDisplay ? "NPS: " + formattedNPS + " (" + maxNPS + ") ~ " : "") + 
						LanguageManager.getTextString('play_score') + formattedSongScore + " ~ " + 
						LanguageManager.getTextString('play_miss') + formattedSongMisses +  " ~ " + 
		...
		else
		{
			switch (SONG.song.toLowerCase())
			{
				...
				case 'exploitation':
					if (FlxG.save.data.compactNumbers) {
					scoreTxt.text =
					(FlxG.save.data.npsDisplay ? "N9S: " + formattedNPS + " (" + maxNPS + ") ~ " : "") +  
					"Scor3: " + (songScore * FlxG.random.int(1,9));
					} else {
						scoreTxt.text =
						(FlxG.save.data.npsDisplay ? "N9S: " + nps + " (" + maxNPS + ") ~ " : "") +  
		...
		if (SONG.song.toLowerCase() != "five-nights" || foePlay)
		{
			if (healthBar.percent < 20)
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent)
				{
					...
					switch (SONG.song.toLowerCase())
					{
						...
						default:
							if (FlxG.save.data.hdrain && health > 0.2)
							{
								if (!daNote.isSustainNote)
									health -= 0.01725;
								else
	...
	function endSong():Void
	{
		...
		FlxG.sound.music.volume = 0;
		vocals.volume = 0;
		if (SONG.validScore && !botPlay && !pMode && !(!modchartoption && (SONG.song.toLowerCase() == 'cheating' || SONG.song.toLowerCase() == 'unfairness' || SONG.song.toLowerCase() == 'kabunga' || localFunny == CharacterFunnyEffect.Exbungo || localFunny == CharacterFunnyEffect.Recurser || SONG.song.toLowerCase() == 'exploitation')))
		{
			...
			else
			{
				switch (curSong.toLowerCase())
				{
					...
					case "electric-cockaldoodledoo":
						...
						CharacterSelectState.unlockCharacter('shartey');
					case "eletric-cockadoodledoo":
		...
		if (isStoryMode)
		{
			...
			if (storyPlaylist.length <= 0)
			{
				...
				if (SONG.validScore && !botPlay && !pMode)
				{
		...
		else
		{
			if (storyDifficulty == 0) {
				...
				if (!botPlay && !pMode) completedSongs.push(curSong);
				for (i in 0...mustCompleteSongs.length)
				{
					if (!completedSongs.contains(mustCompleteSongs[i]))
					{
	...
	function noteMiss(direction:Int = 1, note:Note):Void
	{
		if (true)
		{
			...
			if (dad.animation.getByName('sing${noteToPlay}miss') != null)
				...
				{
					//dad.color = 0xFF000084; i can't fix this fdalkapfamkfamfamkfaMFAD
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (darkLevels.contains(curStage) && SONG.song.toLowerCase() != "polygonized" && SONG.song.toLowerCase() != "too-slow-encore" && formoverride != 'tristan-golden-glowing' && bfTween == null)
			{
				boyfriend.color = nightColor;
			}
			...
			switch (note.noteStyle)
			{
				default:
					...
					boyfriend.playAnim('sing' + fuckingDumbassBullshitFuckYou, true);
						...
						dad.playAnim('sing' + fuckingDumbassBullshitFuckYou, true);
					}
	...
	override function stepHit()
		...
		/*	if (!FreeplayState.isaCustomSong) {
			...
			switch (SONG.song.toLowerCase())
			{
				...
				case 'electric-cockaldoodledoo':
					switch (curStep)
					{
						...
						case 5479:
							FlxG.camera.flash(FlxColor.WHITE, 0.5);
							switchBF('shartey', boyfriend.getPosition(), false);
						case 5501:
							FlxTween.tween(black, {alpha: 0.6}, 1);
				...
				switch (curStep)  
			    {
					...
					botplayTxt.text = 'NOTE: Expunged and bambi are NOT the same person';
					case 840:
						if (!botPlay) {
					...
					dad.color = nightColor;
					switchBF('dave-angey', boyfriend.getPosition(), false);
					boyfriend.color = nightColor;
			    }
				...
				case 400: 
					...
					for (sprite in bambiFarmDream)
						...
						flyingBgsFarm.visible = true;
						defaultCamZoom -= 0.3;
				switchDad('expunged', dad.getPosition(), false);
				//dad.color = nightColor;
				switchBF('dave-annoyed', boyfriend.getPosition(), false);
				//boyfriend.color = nightColor;
				if (!hasTriggeredDumbshit)
				{
					throwThatBitchInThere('expunged', 'bambi-new'); // hope this works
				}
				boyfriend.playAnim('um', true);
				...
				case 928:
					subtitleManager.addSubtitle('I\'m gonna get ya!', 0.11, 0.7);
				case  931:
					makeInvisibleNotes(true);
					FlxTween.tween(boyfriend, {alpha: 0}, 3);
					defaultCamZoom = FlxG.camera.zoom + 0.3;
					FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom + 0.3}, 4);
					case 967:
					subtitleManager.addSubtitle('I am..', 0.03, 0.5);
				case 985:
					subtitleManager.addSubtitle('GOD', 0.02, 0.7, {subtitleSize: 60});
				case 1056:
					defaultCamZoom = FlxG.camera.zoom - 0.3;
					FlxTween.tween(boyfriend, {alpha: 1}, 0.2);
					makeInvisibleNotes(false);
				...
				case 1926:
					...
					defaultCamZoom += 0.2;
					FlxG.camera.flash(FlxColor.WHITE, 1);
					switchDad('bambi-new', dad.getPosition(), true); // so the camera doesn't bug lol // nvm didn't work sdopafdfapo
					dad.visible = false;
					for (sprite in bambiFarmDream)
						{
							sprite.visible = true;
						...
						splitathonExpression('bambi', 'umWhatIsHappening');
						case 1968:
						PlatformUtil.sendFakeMsgBox("I GOT YOU NOW DAVE");
						PlatformUtil.sendWindowsNotification("expunged.dat", "I GOT YOU NOW DAVE");  
			}
			case 'sporting':
				switch (curStep)
				{
				...
				case 'importumania':
					switch (curStep)
					{
						...
						case 9216:
							...
							switchDad('bambi-angey', dad.getPosition(), false); // fix crash
							FlxG.camera.flash(FlxColor.WHITE, 1);
			...
			if (SONG.song.toLowerCase() == 'exploitation' && curStep % 8 == 0)
			{
				...
				kadeEngineWatermark4.font = Paths.font('exploit/${chosenFont}.ttf');
	...
	override function beatHit()
	{
      ...
      if(foePlay) {
		if(curBeat % 2 == 0)
			{
				...
				{
					dad.dance();
					if (darkLevels.contains(curStage) && (SONG.song.toLowerCase() != "polygonized" && (SONG.song.toLowerCase() != "too-slow-encore" || (SONG.song.toLowerCase() == "polygonized" && curBeat >= 608)) && formoverride != 'tristan-golden-glowing' && bambiTween == null))
					{
						dad.color = nightColor;
					}
					else if(sunsetLevels.contains(curStage) && bambiTween == null)
					{
						dad.color = sunsetColor;
					}
					else if (bambiTween == null)
					{
						dad.color = FlxColor.WHITE;
					}
					else
					{
						dad.color = bambiTween.color;
					}
				}
			}
	  }
		if(curBeat % 2 == 0)
		{
			...
			{
				boyfriend.dance();
				if (darkLevels.contains(curStage) && (SONG.song.toLowerCase() != "polygonized" && (SONG.song.toLowerCase() != "too-slow-encore" || (SONG.song.toLowerCase() == "polygonized" && curBeat >= 608)) && formoverride != 'tristan-golden-glowing' && bfTween == null))
				{
					boyfriend.color = nightColor;
				}
				else if(sunsetLevels.contains(curStage) && bfTween == null)
	...
	function makeInvisibleNotes(invisible:Bool)
	{
		if (invisible)
		{
			for (strumNote in strumLineNotes)
			{
				///if (!focusOnDadGlobal && !foePlay) { // so you can see if your playing lol
		...
		else
		{
			for (strumNote in strumLineNotes)
			{
				//if (focusOnDadGlobal && foePlay) { // flsfjosdjsjl
				FlxTween.cancelTweensOf(strumNote);
